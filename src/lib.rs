...
pub struct Combinator<'a, T> where T: 'a {
	seq: &'a [T],
	indices: Vec<usize>,
	inited: bool,
}
...
pub struct CombinationIter<'a, 'b, T> where T: 'a {
...
{
	if len > seq.len() {
		panic!("Combination length longer than sequence ({} > {})", len, seq.len());
	}
...
fn all_combinations_generated()
{
	...
	let combos: BTreeSet<BTreeSet<u32>> = [
	...
	while let Some(combo) = combinator.next() {
		...
		assert!(combos.contains(&c), "{:?} does not contain {:?}", &combos, &c);
	}
}
